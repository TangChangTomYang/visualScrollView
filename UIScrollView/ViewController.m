...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    NSInteger leftIndex = offsetX / SCROLLVIEW_WIDTH;
...
//    rightView.contentX = (SCROLLVIEW_WIDTH + x - ((leftIndex + 1) * SCROLLVIEW_WIDTH));
    ...
    rightView.contentX = -(SCROLLVIEW_WIDTH - AnimationOffset) + (offsetX - (leftIndex * SCROLLVIEW_WIDTH)) /SCROLLVIEW_WIDTH * (SCROLLVIEW_WIDTH - AnimationOffset);
    
    leftView.contentX = ((SCROLLVIEW_WIDTH - AnimationOffset) + (offsetX - ((leftIndex + 1) * SCROLLVIEW_WIDTH))/SCROLLVIEW_WIDTH * (SCROLLVIEW_WIDTH - AnimationOffset));
    
    NSLog(@"leftIndex:%ld--offsetX:%f-------%f",leftIndex, offsetX,leftView.contentX );
